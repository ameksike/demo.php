...
//print_r($devcap);
$bestFormat='';
/*if ($devcap['image_format']['jpg'] == 1)              //---> Antiguo de la libreria original...
  $bestFormat='jpg';
...
if($devcap['image_format']['png'] == 1)                //---> Version Liester, cambios...
{
    $bestFormat='png';
}
else if($devcap['image_format']['gif'] == 1)
...
else if($devcap['image_format']['jpg']== 1)
{
    $bestFormat = 'jpg';
}
else
...
$OutputImageDir="";
   ...
   imagealphablending($DEST_IMAGE, false);  //---> Arreglo Mio(Liester)... //Para poder eliminar los pixeles de color negro y copiar cualquier color o transparencia...
   $fondo = imagecolorallocatealpha($DEST_IMAGE, 0, 0, 0, 100);   //---> Arreglo Mio(Liester)... //Para ponerle transparencia...
   imagesavealpha($DEST_IMAGE, true);   //---> Arreglo Mio(Liester)... //Salvando la imagen con transparencia...

   unlink($OutputImageFullPath);
   $OUTPUT_FILE=$OutputImageDir."Resized/".$outputFileName.".".$OutputFormat;
   
      //Como estaba en la original con imagecopyresized no suabiza los pixel, se utilizo imagecopyresampled que si de claridad y suabizamiento a la imagen...
      if (!imagecopyresampled($DEST_IMAGE, $SRC_IMAGE, 0, 0, 0, 0, $Out_X, $Out_Y, $SRC_X, $SRC_Y)) {
