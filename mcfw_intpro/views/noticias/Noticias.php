...
{
	if($_REQUEST["accion"]=="savenoticia"){
		$userid = Helper::this("User")->getId();
		Helper::this("Noticias")->save(
			$_REQUEST["titulo"],
			$_REQUEST["text"],
			$userid
		);
...
$noticias = Helper::this("Noticias")->listar($_REQUEST);
...
<?php			
	if(is_array($noticias)) foreach($noticias as $i){
		$username = Helper::this("User")->getFullName($i["idUsuario"]);
		$comentario = Helper::this("Noticias")->getComentariosById($i["idNoticias"]);
		echo Helper::this("Noticias")->getPost(
			"<b> Autor: </b> $username <br> 
			<b> Fecha: </b>". $i["fPublicacion"]." <br> 
			<b> Comentarios: </b>". count($comentario)." <br> 
			<b> Titulo: </b> <a href='../blog/Blog.php?id=".
			$i["idNoticias"]."'>" . $i["titulo"]."</a>"
		);	
	}

		
	if(Helper::this("User")->isLogin()) echo "	<br>
			<h3>Crear Tema</h3>


			<form action='../noticias/Noticias.php".$_REQUEST["id"]."' method='post'>

			      <label> Titulo: <input type='text' name='titulo' width='13' /> <br> </label>
			      <label> Publicacion: <textarea name='text' cols='73%' rows='7'></textarea> </label>

			      <input type='hidden' name='accion' value='savenoticia' />		 
